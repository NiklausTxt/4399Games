...
function ensureSchema(env) {
  ...
  schemaPromise = env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS game_users (
    ...
    env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_game_sessions_user_id ON game_sessions(user_id)"),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS game_scores (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER NOT NULL,
    ...
    env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_game_scores_user_updated ON game_scores(user_id, updated_at DESC)"),
  ]).catch((error) => {
    schemaPromise = null;
    throw error;
  });
...
async function currentUser(request, env) {
  ...
  return env.DB.prepare(`SELECT game_users.id, game_users.username
    FROM game_sessions JOIN game_users ON game_users.id = game_sessions.user_id
...
async function createSession(env, userId) {
  ...
  const tokenHash = await sha256(token);
  const now = Math.floor(Date.now() / 1000);
  await env.DB.prepare("INSERT INTO game_sessions (token_hash, user_id, expires_at, created_at) VALUES (?, ?, ?, ?)")
...
async function userSummary(env, user) {
  const stats = await env.DB.prepare(`SELECT COALESCE(MAX(score), 0) AS bestScore,
    ...
    FROM game_scores WHERE user_id = ?`).bind(user.id).first();
  const recent = await env.DB.prepare(`SELECT game_id AS gameId, score, completed, updated_at AS updatedAt
    FROM game_scores WHERE user_id = ? ORDER BY updated_at DESC LIMIT 10`).bind(user.id).all();
  return {
    loggedIn: true,
...
async function handleApi(request, env, url) {
  ...
  if (path === "/api/auth/register" && method === "POST") {
    ...
    const exists = await env.DB.prepare("SELECT id FROM game_users WHERE username = ? COLLATE NOCASE")
    ...
    let result;
    try {
      result = await env.DB.prepare("INSERT INTO game_users (username, password_hash, password_salt, created_at) VALUES (?, ?, ?, ?)")
  ...
  if (path === "/api/auth/login" && method === "POST") {
    ...
    const user = await env.DB.prepare("SELECT id, username, password_hash, password_salt FROM game_users WHERE username = ? COLLATE NOCASE")
      .bind(credentials.username).first();
  ...
  if (path === "/api/auth/logout" && method === "POST") {
    const token = cookies(request)[SESSION_COOKIE];
    if (token) await env.DB.prepare("DELETE FROM game_sessions WHERE token_hash = ?").bind(await sha256(token)).run();
  ...
  if (path === "/api/scores" && method === "POST") {
    ...
    const now = Math.floor(Date.now() / 1000);
    await env.DB.prepare(`INSERT INTO game_scores (user_id, game_id, score, completed, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT(user_id, game_id) DO UPDATE SET
        score = MAX(game_scores.score, excluded.score),
        completed = MAX(game_scores.completed, excluded.completed),
  ...
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/api/")) {
      try {
        ...
        return json({ error: "服务器处理失败，请稍后再试。", code: "INTERNAL_ERROR" }, { status: 500 });
      }
    }
